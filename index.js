...
async function cleanupStaleRecords(env) {
  try {
    const listResponse = await fetch(
      `https://api.cloudflare.com/client/v4/zones/${env.CLOUDFLARE_ZONE_ID}/dns_records?name.endswith=.${env.DOMAIN_SUFFIX}&type=TXT&per_page=3500`,
      {
        headers: {
          'Authorization': `Bearer ${env.CLOUDFLARE_API_TOKEN}`,
          'Content-Type': 'application/json',
    ...
    const existingRecords = (await listResponse.json()).result;
  } catch (error) {
    console.log('cleanupStaleRecords: list records exception', { error: error.message });
    return;
  }
